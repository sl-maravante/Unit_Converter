...
convertBtn.addEventListener("click",function(){
   ...
   lengthMetric.textContent=`${inputBtn.value} meters = ${lengthInFeet} feet | ${inputBtn.value} feet = ${lengthInMeter} meters` 

    const lengthInGallon=(Number(inputBtn.value)*0.264).toFixed(3)
   const lengthInLiter=(Number(inputBtn.value)/0.264).toFixed(3)
   volumeMetric.textContent=`${inputBtn.value} liter = ${lengthInGallon} gallons | ${inputBtn.value} gallons = ${lengthInLiter} liters` 

    const lengthInPound=(Number(inputBtn.value)*2.204).toFixed(3)
   const lengthInKg=(Number(inputBtn.value)/2.204).toFixed(3)
   massMetric.textContent=`${inputBtn.value} kilograms = ${lengthInPound} pound | ${inputBtn.value} pound = ${lengthInKg} Kilograms` 
   inputBtn.value=""
})